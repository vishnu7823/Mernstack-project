...
function HospitalList() {
  ...
  useEffect(() => {
    fetch('https://mernstack-project-bj3q.onrender.com/api/v1/hospitals')
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this hospital?')) {
      try {
        const response = await fetch(`https://mernstack-project-bj3q.onrender.com/api/v1/hospitals/delete?id=${id}`, {
          method: 'DELETE',
        });
