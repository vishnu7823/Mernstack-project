...
const EditHospital = () => {
  ...
  useEffect(() => {
    const fetchHospital = async () => {
      try {
        const response = await fetch(`https://mernstack-project-bj3q.onrender.com/api/v1/hospitals/details?id=${id}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://mernstack-project-bj3q.onrender.com/api/v1/hospitals/update?id=${id}`, {
        method: 'PUT',
